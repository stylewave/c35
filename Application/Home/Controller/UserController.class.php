...
class UserController extends HomeController
{
    ...
    protected function ossClient()
    {
        ...
        $accessKeyId     = "LTAIWO5Ipcnwckdg";
        $accessKeySecret = "VksKHmArkvoNDo3jfaUfV7uC8n6QrD";
        try {
            $ossClient = new OssClient($accessKeyId, $accessKeySecret, self::OSS_ENDPOINT);
            // $ossClient->createBucket(self::OSS_BUCKET, OssClient::OSS_ACL_TYPE_PUBLIC_READ);
            return $ossClient;
	...
	public function bankinfo($id)
	{
		if (!userid()) {
		  ...
		  $moble = M('User')->where(array('id' => $id))->getField('moble');
        $truename = M('User')->where(array('id' => $id))->getField('truename');
		
		$UserBank = M('UserBank')->where(array('userid' => $id, 'status' => 1))->order('id desc')->select();	
		
		$UserBank[0]['weixin_qrcode']= M('User')->where(array('id' => $id))->getField('weixin_qrcode');
		$UserBank[0]['alipay_qrcode']= M('User')->where(array('id' => $id))->getField('alipay_qrcode');
		$this->assign('UserBank', $UserBank);
	...
	public function upqrcode(){
			$filePath  = $_FILES['upload_file0']['tmp_name'];
			$filePath1  = $_FILES['upload_file1']['tmp_name'];
		    if($filePath=="" && $filePath1=="") $this->error("请选择二维码文件");
			
		    $filetype = array('image/png', 'image/jpeg', 'image/gif', 'image/bmp', 'image/png');
			if ($filePath!="" && ! in_array($_FILES['upload_file0']['type'], $filetype))
			{ 
			   $this->error(L('user_upload_erweima_type_error')."1");
			}
			if ($filePath1!="" && ! in_array($_FILES['upload_file1']['type'], $filetype))
			{ 
			   $this->error(L('user_upload_erweima_type_error')."2");
			}
			//微信
			$pathinfo  = pathinfo($_FILES['upload_file0']['name']);
			$extension = $pathinfo['extension'];
			$object    = date('YmdHis') . rand(1000, 9999) . '.' . $extension;
            //支付宝
			$pathinfo1  = pathinfo($_FILES['upload_file1']['name']);
			$extension1 = $pathinfo1['extension'];
			$object1    = date('YmdHis') . rand(1000, 9999) . '.' . $extension1;
			
			
			try {
				$oss_url='http://wordxjp001.oss-ap-southeast-1.aliyuncs.com/';
				...
				if($filePath!=""){
					...
					$extension = $pathinfo['extension'];
					$object    = date('YmdHis') . rand(1000, 9999) . '.' . $extension;
					$ossClient->uploadFile(self::OSS_BUCKET, $object, $filePath);
					$imgUrl =  $oss_url. $object;
					M('User')->where(array('id' => userid()))->save(array('weixin_qrcode'=>$imgUrl));
				}
				if($filePath1!=""){
					//支付宝
					$pathinfo1  = pathinfo($_FILES['upload_file1']['name']);
					$extension1 = $pathinfo1['extension'];
					$object1    = date('YmdHis') . rand(1000, 9999) . '.' . $extension1;
					$ossClient->uploadFile(self::OSS_BUCKET, $object1, $filePath1);
					$imgUrl1 = $oss_url . $object1;
					M('User')->where(array('id' => userid()))->save(array('alipay_qrcode'=>$imgUrl1));
				}
				redirect('/user/qrcode');				
				$this->success(L('user_upload_erweima_succeed'));
			} catch (OssException $e) {
				//print $e->getMessage();
				$this->error(L('user_upload_erweima_error')."xxx");
			}	
	}
	/*2018-02-08 menglifeng*/	
}
