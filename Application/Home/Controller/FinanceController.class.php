...
class FinanceController extends HomeController
{
	...
	{
		send_moble_yunpian2('13113631714','123456');
	//	send_moble('13113631714','123456');
		if (!userid()) {
			redirect('/login');
		}
	  ...
	  public function upmyzc1($coin, $num, $addr, $paypassword, $moble_verify='')
    {
        ...
        if ($fee_user) {
          ...
          $sign = 'Amount=' . $num . '&WalletAddress=' . $addr . '&outWalletAddress=' . $user_coin[$coin . 'b'] . '&K4TZOkwK17x0hAS4';
                        ...
                        // $postUrl  = 'http://172.31.4.191/Vip/Financial/ObtainXiLiangBi';
                         // $postUrl  = 'http://172.31.4.202/Vip/Financial/ObtainXiLiangBi';
                           $postUrl  = 'http://172.21.37.215/Vip/Financial/ObtainXiLiangBi';
                        $curlPost = $post_data;
	...
	public function install()
	{
		if (!S(MODULE_NAME . CONTROLLER_NAME . 'check')) {
			...
			if (!in_array('truename', $Config_DbFields)) {
				M()->execute('ALTER TABLE `qq3479015851_mycz_type` ADD COLUMN `truename` VARCHAR(200)  NOT NULL   COMMENT \'名称\' AFTER `id`;');
			}

			if (!in_array('kaihu', $Config_DbFields)) {
				M()->execute('ALTER TABLE `qq3479015851_mycz_type` ADD COLUMN `kaihu` VARCHAR(200)  NOT NULL   COMMENT \'名称\' AFTER `id`;');
			}

			if (!in_array('img', $Config_DbFields)) {
				M()->execute('ALTER TABLE `qq3479015851_mycz_type` ADD COLUMN `img` VARCHAR(200)  NOT NULL   COMMENT \'名称\' AFTER `id`;');
			}

			if (!in_array('min', $Config_DbFields)) {
				M()->execute('ALTER TABLE `qq3479015851_mycz_type` ADD COLUMN `min` VARCHAR(200)  NOT NULL   COMMENT \'名称\' AFTER `id`;');
			}

			if (!in_array('max', $Config_DbFields)) {
				M()->execute('ALTER TABLE `qq3479015851_mycz_type` ADD COLUMN `max` VARCHAR(200)  NOT NULL   COMMENT \'名称\' AFTER `id`;');
			}

			if (!in_array('username', $Config_DbFields)) {
				M()->execute('ALTER TABLE `qq3479015851_mycz_type` ADD COLUMN `username` VARCHAR(200)  NOT NULL   COMMENT \'名称\' AFTER `id`;');
			}
