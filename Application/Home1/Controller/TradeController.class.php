...
class TradeController extends HomeController
{
    ...
    public function ofcbuy($id)
    {
        ...
        if ($moble) {
            ...
            // $this->error('请先绑定手机');
            $this->error('请先绑定手机',U('Home/User/moble'));
        }
        $UserBank = M('UserBank')->where(array('userid' => userid(), 'status' => 1))->order('id desc')->limit(1)->select();
            if (!$UserBank) {

                 $this->error('请先绑定银行卡',U('Home/User/bank'));
            }
        if (!check($id, 'd')) {
            $this->error('请选择要卖出的委托');
        }
        ...
        $t = M('trade')->where(' userid=' . userid() . ' and (status=0 ) and market=\'ppc_usd \'  ')->count();
        if ($t >= 1) {
            $this->error('您上一单们还没有成交，成交后在继续操作');
        }
...
/* 				if ($Market[$v['name'] . '_cny']['new_price']) {
        ...
        //户获得所有未成交的买单
        if (!userid()) {
            $result1 = M()->query('select userid, id,ROUND(price,2) as price,num,deal,mum,type,fee,status,addtime from movesay_trade where status=0 and status1 is null and type=2 and market=\'' . $market . '\'  order by price asc limit 10;');

        } else {
            $result1 = M()->query('select userid, id,ROUND(price,2) as price,num,deal,mum,type,fee,status,addtime from movesay_trade where status=0 and status1 is null and type=2 and market=\'' . $market . '\' and userid<>' . userid() . ' order by price asc limit 10;');
            // $result1 = M()->query('select userid, id,ROUND(price,2) as price,num,deal,mum,type,fee,status,addtime from movesay_trade where status=0   order by price asc limit 10;');

        }
        $buy = M()->query('select id,ROUND(price,2) as price,(num-deal)as nums from movesay_trade where status=0 and type=1 and market =\'' . $market . '\'  order by price desc limit 10;');
        $this->assign('tradelistbuy', $buy);
        $this->assign('tradelist', $result1);
		...
		//新增的 2018-1-6
		$this->assign('integral', $UserCoin['integral']);
    ...
    public function upTrade2($paypassword = null, $id)
    {
        ...
        $tt    = time() - 3600 * 48;
        $List1 = $mo->table('movesay_trade')->where("market='ppc_usd' and status=0 and userid=" . $t['userid'] . " and  status1=2 and  paytime< " . $tt)->count();
