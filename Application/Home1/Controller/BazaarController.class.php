...
//dump($list);die;
	...
	public function sell() {
		...
		$rs[] = $mo->table('qq3479015851_bazaar')->add(array('userid' => $uid,'coin'=>$xnb, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => $bazaar_config['fee'], 'addtime' => time(), 'status' => 0));
	...
	public function buy($id = NULL, $num = NULL, $coin = NULL, $paypassword = NULL)
    {
        ...
        $mo = M();
        $mo->execute('set autocommit=0');
        $mo->execute('lock tables qq3479015851_user_coin write  , qq3479015851_bazaar write , qq3479015851_bazaar_log write , qq3479015851_invit write');

        $rs = array();
        /*执行交易*/
        $rs[] = $mo->table('qq3479015851_user_coin')->where(array('userid' => $user['id']))->setDec($rmb, $total);//买家减少余额
        //$rs[] = $mo->table('qq3479015851_user_coin')->where(array('userid' => $bazaar['userid']))->setDec($xnb, $total);//卖家减少虚拟币
        $rs[] = $mo->table('qq3479015851_user_coin')->where(array('userid' => $bazaar['userid']))->setInc($rmb, round($total - $fee, 8));//卖家增加余额
        $rs[] = $mo->table('qq3479015851_user_coin')->where(array('userid' => $user['id']))->setInc($xnb, $num);//买家增加虚拟币
        /*添加记录*/
        $rs[] = $mo->table('qq3479015851_bazaar_log')->add(array('userid' => $user['id'], 'peerid' => $bazaar['id'], 'coin' => $xnb, 'price' => $bazaar['price'], 'num' => $num, 'mum' => $total, 'fee' => $fee, 'addtime' => time(), 'status' => 1));
        /*改变委托剩余数量*/
        $rs[] = $mo->table('qq3479015851_bazaar')->where(array('id' => $id))->setInc(deal, $num);
        /*改变委托状态*/
        $bazaar_sell = M('Bazaar')->where(array('id' => $id))->find();
        if ($bazaar_sell['deal'] == $bazaar_sell['num']) {
            $sell = $mo->table('qq3479015851_bazaar')->where(array('id' => $id))->save(array('status' => 1));
        }
        ...
        if ($tj_1) {
            if ($tj_1 > 0 && isset($fee_1)) {
                ...
                if ($tj_fee_1) {
                    $rs[] = $mo->table('qq3479015851_user_coin')->where(array('userid' => $tj_1))->setInc($rmb, $tj_fee_1);//增加提成
                    $rs[] = $mo->table('qq3479015851_invit')->add(array('userid' => $tj_1, 'invit' => $user['id'], 'name' => '一代买入赠送', 'type' => $market . '集市交易赠送', 'num' => $num, 'mum' => $total, 'fee' => $tj_fee_1, 'addtime' => time(), 'status' => 1));//增加提成记录
                }
            }
                /*二代*/
            if($tj_2){
                if ($tj_2 > 0 && isset($fee_2)) {
                    ...
                    if ($tj_fee_2) {
                        $rs[] = $mo->table('qq3479015851_user_coin')->where(array('userid' => $tj_2))->setInc($rmb, $tj_fee_2);
                        $rs[] = $mo->table('qq3479015851_invit')->add(array('userid' => $tj_2, 'invit' => $user['id'], 'name' => '二代买入赠送', 'type' => $market . '集市交易赠送', 'num' => $num, 'mum' => $total, 'fee' => $tj_fee_2, 'addtime' => time(), 'status' => 1));
                    }
                }
                 /*三代*/
                if ($tj_3) {
                    if ($tj_3 > 0 && isset($fee_3)) {
                        ...
                        if ($tj_fee_3) {
                           $rs[] = $mo->table('qq3479015851_user_coin')->where(array('userid' => $tj_3))->setInc($rmb, $tj_fee_3);
                           $rs[] = $mo->table('qq3479015851_invit')->add(array('userid' => $tj_3, 'invit' => $user['id'], 'name' => '三代买入赠送', 'type' => $market . '集市交易赠送', 'num' => $num, 'mum' => $total, 'fee' => $tj_fee_3, 'addtime' => time(), 'status' => 1));
                        }
                    }
                }
...
//dump($list);die;
    ...
    public function mywtUp($market, $num, $price, $paypassword)
    {
        ...
        $rs[] = $mo->table('qq3479015851_user_coin')->where(array('userid' => $uid))->setDec($xnb, $num);
        $mum = round($num * $price, 8);
        $rs[] = $mo->table('qq3479015851_bazaar')->add(array('userid' => $uid, 'market' => $market, 'coin'=>$xnb, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => $bazaar_cof['fee'], 'addtime' => time(), 'status' => 0));
...
//		}
	...
	public function install()
	{
		...
		if (in_array('bazaar', $game_arr)) {
			...
			if (!isset($tableMap['qq3479015851_bazaar'])) {
